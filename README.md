...
「撮る → 気づいたら共有用URLがクリップボードにある」を実現するための、個人〜小規模チーム向けの軽量セルフホスト共有基盤です。バックエンドはPHP + SQLiteのみで動作し、フレームワークやDBサーバーのセットアップは不要。VPSや共有レンタルサーバーにそのまま置けます。
...
| レイヤー | 技術 |
| :--- | :--- |
| クライアント | C# / WPF (`net10.0-windows`)、[CommunityToolkit.Mvvm](https://github.com/CommunityToolkit/dotnet)、[H.NotifyIcon.Wpf](https://github.com/HavenDV/H.NotifyIcon)（タスクトレイ常駐） |
...
- Windows 10/11
- [.NET 10 デスクトップランタイム](https://dotnet.microsoft.com/download/dotnet/10.0)（クライアントはフレームワーク依存配布のため、実行対象マシンに別途インストールが必要です。自己完結型配布ではありません）
...
発行後の成果物は `Client/bin/Release/net10.0-windows/win-x64/publish/JustLinkIt.Client.exe` に出力されます。このフォルダを対象マシンにそのままコピーして配布できます（対象マシンに.NET 10デスクトップランタイムが必要）。
